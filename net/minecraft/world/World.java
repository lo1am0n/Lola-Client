...
public abstract class World implements IBlockAccess
{
    ...
    public MovingObjectPosition rayTraceIgnore(Vec3 vec31, Vec3 vec32, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock, Class ignore)
    {
        if (!Double.isNaN(vec31.xCoord) && !Double.isNaN(vec31.yCoord) && !Double.isNaN(vec31.zCoord))
        {
            if (!Double.isNaN(vec32.xCoord) && !Double.isNaN(vec32.yCoord) && !Double.isNaN(vec32.zCoord))
                ...
                if ((!ignoreBlockWithoutBoundingBox || block.getCollisionBoundingBox(this, blockpos, iblockstate) != null) && block.canCollideCheck(iblockstate, stopOnLiquid))
                {
                    ...
                    if (movingobjectposition != null)
                    {
                        if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY) {
                            if (movingobjectposition.entityHit.getClass() != ignore) {
                                return movingobjectposition;
                            }
                        }
                        else {
                        ...
                        if (block1.canCollideCheck(iblockstate1, stopOnLiquid))
                        {
                            ...
                            if (movingobjectposition1 != null)
                            {
                                if (movingobjectposition1.typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY) {
                                    if (movingobjectposition1.entityHit.getClass() != ignore) {
                                        return movingobjectposition1;
                                    }
                                }
                                else {
